...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Fetching consumptions for user: ${user.id}, all_locations: ${allLocations}, location_id: ${locationId}`)
    ...
    if (!allLocations && activeLocationId) {
      consumptionsQuery = consumptionsQuery.eq('location_id', activeLocationId)
    } else if (!allLocations && userGroupId) {
      // If no specific location but not all locations, filter by user's group
      const { data: groupLocations, error: groupLocationsError } = await supabase
        .from('locations')
        .select('id')
        .eq('group_id', userGroupId)

      if (groupLocationsError) {
        console.error('Error fetching group locations:', groupLocationsError)
        return new Response(
          JSON.stringify({ error: 'Failed to fetch group locations' }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }

      const locationIds = groupLocations.map(loc => loc.id)
      consumptionsQuery = consumptionsQuery.in('location_id', locationIds)
    }

    const { data: consumptions, error: consumptionsError } = await consumptionsQuery

    if (consumptionsError) {
      console.error('Error fetching consumptions:', consumptionsError)
      return new Response(
        JSON.stringify({ error: 'Failed to fetch consumptions' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }

    console.log(`Retrieved ${consumptions?.length || 0} consumptions`)
    ...
    console.error('Unexpected error in consumptions function:', error)
    return new Response(
